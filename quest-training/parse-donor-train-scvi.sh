...
#SBATCH --job-name=scvi-parse-donor
#SBATCH --output=scvi_parse_donor_%j.out
#SBATCH --error=scvi_parse_donor_%j.err
...
# --- Job params ---
PROJECT_ROOT=/projects/p32222/state-reproduce/          # repo checkout
DATA_TOML=${PROJECT_ROOT}/latest-tomls/donor.toml  # the file you showed
OUTPUT_DIR=/projects/p32222/state_runs/scvi_parse_donor/          # large shared storage
mkdir -p "${OUTPUT_DIR}"

cd "${PROJECT_ROOT}"

# --- Launch (single-GPU lightning trainer) ---
srun python -m baselines.state_sets_reproduce.train \
